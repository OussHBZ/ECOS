...
def create_simple_consultation_pdf(conversation, case_number, evaluation_results, recommendations=None):
    """Create a simplified PDF document without advanced visualizations"""
    ...
    # Add score summary at the top (simplified version without gauge)
    ...
    if recommendations and len(recommendations) > 0:
        # Vérifier si les recommandations semblent être en anglais et les traduire si nécessaire
        translated_recommendations = []
        english_indicators = ['here are', 'practice', 'review', 'improve', 'make sure', 'remember to']
        
        for rec in recommendations:
            # Vérifier si la recommandation semble être en anglais
            if any(indicator in rec.lower() for indicator in english_indicators):
                # Dans un cas réel, on utiliserait ici une API de traduction
                # Mais pour simplifier, nous utilisons des messages d'erreur explicites
