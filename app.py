...
class ConsultationPDF(BaseDocTemplate):
    def __init__(self, filename):
        ...
        def add_header_footer(canvas, doc):
            ...
            header_text = f"Consultation OSCE - Rapport"
...
def create_consultation_pdf(conversation, case_number, evaluation):
    """Create a PDF document from the consultation conversation with enhanced evaluation"""
    ...
    # Add title
    elements.append(Paragraph(f"Consultation - Cas {case_number}", title_style))
    ...
    # Key fix: Process only non-system messages and preserve the chronological order
    # The order in the session should be the actual chronological order of the conversation
    ...
    for msg in conversation_messages:
        ...
        # Format message with role only (no timestamp)
        message_text = f"<b>{role}:</b> {content}"
    ...
    # Add evaluation section if provided
    if evaluation:
        elements.append(PageBreak())
        elements.append(Paragraph("Évaluation", title_style))
        
        # Calculate score
        total_points = evaluation.get('points_total', 0)
        earned_points = evaluation.get('points_earned', 0)
        percentage = evaluation.get('percentage', 0)
        
        # If new format doesn't provide these, calculate them
        if total_points == 0 and evaluation.get('checklist'):
            total_points = sum(item.get('points', 1) for item in evaluation.get('checklist', []))
            earned_points = sum(item.get('points', 1) for item in evaluation.get('checklist', []) if item.get('completed', False))
            percentage = round((earned_points / total_points) * 100) if total_points > 0 else 0
        
        # Add score summary
        elements.append(Paragraph(f"Score: {earned_points}/{total_points} ({percentage}%)", subtitle_style))
        elements.append(Spacer(1, 10))
        
        # Add checklist items
        if evaluation.get('checklist'):
            elements.append(Paragraph("Grille d'évaluation:", subtitle_style))
            
            for item in evaluation['checklist']:
                status = "✅ Réalisé" if item.get('completed', False) else "❌ Non réalisé"
                points = item.get('points', 1)
                earned = points if item.get('completed', False) else 0
                
                item_text = f"{item.get('description', 'Item')} - {status} ({earned}/{points} points)"
        ...
        if evaluation.get('feedback'):
            ...
            elements.append(Paragraph(evaluation['feedback'], normal_style))
